...
test("requires complete, versioned metadata on every runtime guide target", async () => {
  const source = await readProjectFile("src/OperationsApp.tsx");
  ...
  while (markerIndex >= 0) {
    ...
    assert.ok(tagStart >= 0 && tagEnd > markerIndex, `Could not parse guided target near character ${markerIndex}.`);
