...
Staging has Supabase Auth profiles, RLS, `account_creation_audit`, `live_chat_messages`, `direct_messages`, `message_logs`, and an active `manager-create-account` Edge Function for owner-created staff, student, and parent profiles. Public signup is disabled in hosted Auth (`disable_signup=true`); email auth and confirmation remain enabled for administrator invitations and existing-user activation links. No-auth function calls return 401, and the function validates the bearer token, Supabase Auth user, active staff-owner profile, requested role, and password policy before using the service role internally. The `twilio_messaging_relay` migration has been applied to staging, creating `sms_consent_records` and `twilio_relay_attempts`, and the hosted `twilio-messaging` Edge Function is deployed with `verify_jwt = false` for Twilio webhooks. Hosted Twilio secrets are not set yet because `supabase secrets set/list --project-ref zfuwbbepsnmmlpgfkmhz` currently returns `403` / `Your account does not have the necessary privileges to access this endpoint`. The supported administrator sign-ins are `Manager1` and `Dev123`, both active staff owners; `Manager123` is retained only as a username alias for the live `Manager1` identity. Their internal Auth emails are `manager1@accounts.chosmartialarts.app` and `dev123@accounts.chosmartialarts.app`. Other profiles are administrator-created and activate by invitation or the existing-account-only login-page activation link.
