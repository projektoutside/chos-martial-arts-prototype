...
Managers can click **Check Relay Health** after entering a Relay health check URL, or leave the field empty to use the staging Supabase relay health URL. For the configured Supabase Function endpoint, the browser sends `credentials: "omit"` with `Accept: application/json` and adds the Supabase bearer token only when a Supabase Auth session is available; private same-site relay URLs still use `credentials: "include"` for server-managed sessions. The private relay verifies the manager session, server-held Twilio environment variables, sender compliance, and webhook configuration without returning secret values. The app validates that response with `validateTwilioRelayHealthResponseForBrowser`; a simple `{ "status": "ready" }` or `{ "ok": true }` response is intentionally rejected because it does not prove the production relay checked the required server-side gates. Private server code can use `buildChoMessagingServerHealthResponse` from `src/messagingServerContract.ts` to create the browser-safe response shape.
