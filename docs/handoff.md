...
- Verified staging URL: `https://chos-martial-arts-operations-app.pages.dev/`
- Deployment proof path: `.github/workflows/deploy-pages.yml` verifies `main`; authenticated Wrangler Direct Upload publishes the verified artifact to Cloudflare Pages
- Staging build configuration: GitHub repo variables and the approved local release environment provide browser-safe `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`, and `VITE_ENABLE_DEVELOPER_ACCOUNT=true`; builds fail if the Supabase URL does not point at Cho's staging project `zfuwbbepsnmmlpgfkmhz`
...
Until the hosted Supabase relay secrets and Twilio 10DLC setup are verified, support should focus on staging manager auth, owner-created pilot accounts, signed-in Supabase app-state behavior, Cloudflare Pages deployment health, and documented provider-readiness gaps. Do not promise a production-ready staff account rollout, production auth, payments, email, or live SMS from the current hosted app.
